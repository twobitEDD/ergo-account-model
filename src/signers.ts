import type {
  ...
  ResolveAccountSigners,
  SignerResolutionInput,
} from "./types";
...
const buildReadonlySigner = (reasonUnavailable: string): AccountSigner => ({
  id: "readonly",
  provider: "none",
...
export const buildDefaultSigners: ResolveAccountSigners = (
  input: SignerResolutionInput
): AccountSigner[] => {
  ...
  if (signers.length === 0) {
    if (input.nautilusApiAvailable) {
      signers.push(
        buildReadonlySigner("Nautilus API is available but not currently connected.")
      );
    } else {
...
export const resolveSignerSnapshot = (signers: AccountSigner[]): AccountSignerSnapshot => {
  const safeSigners = signers.length > 0 ? signers : [buildReadonlySigner("No signer configured.")];
  const activeSigner = safeSigners.find((signer) => signer.canSign) ?? null;
  return {
    activeSignerId: activeSigner?.id ?? null,
    signers: safeSigners,
  };
};
