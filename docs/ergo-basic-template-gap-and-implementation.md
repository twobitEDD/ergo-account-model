...
- `@ergo-games/account-model` did not include an explicit signer abstraction.
...
- Added signer abstraction types and resolver contract to `@ergo-games/account-model`:
  - `src/types.ts`
  - `src/signers.ts`
  - `src/provider.tsx`
  - `src/index.ts`
